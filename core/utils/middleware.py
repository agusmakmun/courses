...
# from sentry_sdk import capture_exception  # this module not needed yet
...
class TokenExpirationMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        if hasattr(request, 'auth'):
            if request.auth and request.auth.was_expired:
                response_data = {'status': 401,
                                 ...
                                 'result': {
                                     ...
                                     'expiration_date': str(request.auth.expiration_date),
                                     'was_expired': request.auth.was_expired
                                 }}
